...
def processar_compra(call):
    ...
    bot.edit_message_text("⏳ *A procurar número disponível...*", call.message.chat.id, call.message.message_id)
        ...
        if r.status_code == 200:
            ...
            for _ in range(25):
                ...
                if c.get('sms'):
                    ...
                    return
            bot.send_message(call.message.chat.id, "⚠️ Tempo esgotado. Verifique o número ou tente outro.")
        else:
            erro = res.get('errors', ['Sem stock'])[0]
            bot.send_message(call.message.chat.id, f"❌ *FORNECEDOR:* {erro}\n\n💡 Tente Inglaterra ou EUA se Portugal falhar.")
            
    except requests.exceptions.Timeout:
        bot.send_message(call.message.chat.id, "⚠️ O servidor do 5sim está lento. Tente novamente em 10 segundos.")
    except Exception:
        bot.send_message(call.message.chat.id, "⚠️ Ocorreu um erro na ligação. Tente novamente.")
